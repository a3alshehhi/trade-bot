...
# -*- coding: utf-8 -*-
"""
sd_autotrade_stable.py — «بوت المستقر»: محرّك تنفيذ بوت العرض/الطلب الرئيسي بحالته
قبل تعديلات 10-07-2026 (= لقطة 09-07، commit dbd86c6): بلا بوابة حجم touchVolZ،
بلا تعادل-بعد-العمولتين/حارس غبار minOrderAmt، بلا حارس ملاحقة SD_MAX_CHASE_R.
يحتوي على SD_ARM_ON_HIGH (تسليح +1R بالقمة، أُضيف 09-07) + أرضية وقف عند الدخول في
التتبّع (أُضيفت بطلب بو محمد 2026-07-13: بعد التسليح لا ينزل الوقف تحت الدخول).
يستورد sd_bot الحيّ (دوال التيليجرام/الشموع عامّة؛ والإشارات بلا بوابة الحجم = ما قبل 10-07)
ويعمل بدفاتر/حالة/وسم منفصلة (SD_LABEL="SD Stable") عبر وركفلو sd_stable_15m.yml على
Bybit Testnet + Binance Demo — مستقل تماماً عن الرئيسي الحيّ ولا يتأثر بتحديثاته.
=============================================================================
...
def _manage_one(sym, positions):
    ...
    if not pos.get("armed") and arm_px >= entry + R:
        ...
        changed = True
    if pos.get("armed"):
        # أرضية الوقف: بعد التسليح لا ينزل الوقف تحت سعر الدخول أبداً (النص الثاني بلا خسارة USDT).
        floor_price = entry
